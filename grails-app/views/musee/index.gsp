...
<body>
	...
	<div id="list-musee" class="content scaffold-list" role="main">
		...
		<g:form>
			<fieldset class="form">
				<div class="field-inline">
					<label for="nom"> <g:message code="musee.form.nom.label"
							default="Nom du musée :" />
					</label>
					<g:textField name="nom" value="${param.nom}" />
				</div>
				<div class="field-inline">
					<label for="rue"> <g:message code="musee.form.rue.label"
							default="Rue du musée :" />
					</label>
					<g:textField name="rue" value="${param.rue}" />
				</div>
				<div class="field-inline">
					<label for="codePostale"> <g:message
							code="musee.form.code.label" default="Code postal :" />
					</label>
		...
		<g:if test="${showMusee}">
			<p class="num-result">
				${museeInstanceCount}
				<g:message code="musee.resultat.label" default="résultats" />
			</p>
				...
				<div class="listing-musee">
					...
					<p>
						<strong><g:message code="musee.adresse.label"
								default="Adresse" /></strong> <span class="capital">
					...
					<g:if test="${museeInstanceCount > 2}">
						<div id="actionFav_${museeInstance.id}" style="float: right">
							<g:if
								test="${session.getAttribute("favoris") && session.getAttribute("favoris").get(museeInstance.id)}">
								<input id="added_${museeInstance.id}" disabled="disabled"
									type="button" value="Ajouter à ma liste de musées" />
							</g:if>
							<g:else>
								<g:render template="formAddFavoris"
									model="['id': museeInstance.id, 'nom': fieldValue(bean: museeInstance, field: 'nom')]" />
							</g:else>
						</div>
					</g:if>
				</div>
